...
int main(int argc, char **argv) {
    ...
    const char filename[] = "bpflp_igmp_start_timer_kern.o";
    ...
    prog_fentry = bpf_object__find_program_by_name(obj, "igmp_start_timer_fentry");
    if (!prog_fentry) {
        fprintf(stderr, "Error: finding program 'igmp_start_timer_fentry' in the object file failed\n");
        goto cleanup;
    }

    prog_fmod_ret = bpf_object__find_program_by_name(obj, "igmp_start_timer_fmod_ret");
    if (!prog_fmod_ret) {
        fprintf(stderr, "Error: finding program 'igmp_start_timer_fmod_ret' in the object file failed\n");
        goto cleanup;
    }

    prog_fexit = bpf_object__find_program_by_name(obj, "igmp_start_timer_fexit");
    if (!prog_fexit) {
        fprintf(stderr, "Error: finding program 'igmp_start_timer_fexit' in the object file failed\n");
        goto cleanup;
    }
    ...
    if (libbpf_get_error(link_fentry)) {
        fprintf(stderr, "Error: attaching program 'igmp_start_timer_fentry' failed: %ld\n", libbpf_get_error(link_fentry));
        link_fentry = NULL;
        goto cleanup;
    } else {
        printf("Program 'igmp_start_timer_fentry' attached successfully\n");
    }

    link_fmod_ret = bpf_program__attach(prog_fmod_ret);
    if (libbpf_get_error(link_fmod_ret)) {
        fprintf(stderr, "Error: attaching program 'igmp_start_timer_fmod_ret' failed: %ld\n", libbpf_get_error(link_fmod_ret));
        link_fmod_ret = NULL;
        goto cleanup;
    } else {
        printf("Program 'igmp_start_timer_fmod_ret' attached successfully\n");
    }

    link_fexit = bpf_program__attach(prog_fexit);
    if (libbpf_get_error(link_fexit)) {
        fprintf(stderr, "Error: attaching program 'igmp_start_timer_fexit' failed: %ld\n", libbpf_get_error(link_fexit));
        link_fexit = NULL;
        goto cleanup;
    } else {
        printf("Program 'igmp_start_timer_fexit' attached successfully\n");
    }
